...
public sealed class AppInsightsService
{
    ...
    /// <summary>
    /// Queries Azure Monitor metrics for the Application Insights resource.
    /// </summary>
    public async Task<string> QueryMetricsAsync(string metricName, TimeSpan? timespan = null, CancellationToken ct = default)
    {
        var ts = timespan ?? TimeSpan.FromHours(1);
        ...
        if (metric is null)
            return JsonSerializer.Serialize(new { error = "No metric data found" });

        var points = metric.TimeSeries
            .SelectMany(ts => ts.Values)
            .Select(v => new { timestamp = v.TimeStamp, average = v.Average, min = v.Minimum, max = v.Maximum })
            .ToList();
